...
//const uint16_t ENERGY_TIMESCALES[NUMTIMEFRAMES] PROGMEM = {1,2,4,8,16,32,64,128,256,512,1024};
...
bool busy = FALSE;
	...
	bool predict_energy ()
	{
		...
		//consumption = call IConsumeModel.getConsumePrediction(ETIMESCALE(timeframe), load, state, grade);
	...
	command result_t IEval.reeval_energy_level()
	{
		if (busy)
		{
			return FAIL;
	...
	command result_t IEval.reportPathDone(uint16_t pathnum, uint8_t state, uint32_t energy)
	{
		result_t res;
		...
		{
			res = rcombine(res, call IConsumeModel.pathDone(pathnum, state, energy));
		}
		return res;
	}
}
