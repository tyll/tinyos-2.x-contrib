...
public class SGHelperGenerator {

	// private static ProgramGraph programGraph;

	protected static Hashtable<String, Boolean> defined = new Hashtable<String, Boolean>();
	...
	public static void generateTimedSourceInit(ProgramGraph g,
	...
	public static Vector<String> getTypeType(ProgramGraph g,
	...
	public static Vector<String> getAllTypeTypes(ProgramGraph g,
			TypeDeclaration td) {
	...
	public static void printCallParamDecl(TaskDeclaration td, PrintWriter out) {
	...
	public static void generateTimedSourceConfig(TaskDeclaration td,
			VirtualDirectory out) {
		String name = td.getName();
		String mName = name + "M";
		String interfaceName = "I" + name;
	...
	public static void generateNodeModuleHeader(TaskDeclaration td,
			VirtualDirectory out) {
		String name = td.getName();
		String mName = name + "M";
		String interfaceName = "I" + name;
	...
	public static void generateTimedSourceModuleHeader(TaskDeclaration td,
			VirtualDirectory out) {
		String name = td.getName();
		String mName = name + "M";
		String interfaceName = "I" + name;
	...
	public static void generateNodeInterface(TaskDeclaration td,
			VirtualDirectory out) {
		String name = td.getName();
		String interfaceName = "I" + name;
		String filename = interfaceName + ".nc";
	...
	public static void generateNodeConfig(TaskDeclaration td,
			VirtualDirectory out) {
		String name = td.getName();
		String mName = name + "M";
		String interfaceName = "I" + name;
	...
	public static void generateNodeModule(TaskDeclaration td,
			VirtualDirectory out) {
		String name = td.getName();
		String mName = name + "M";
		String interfaceName = "I" + name;
	...
	public static void generateErrHandlerModuleHeader(ErrorHandler eh,
			VirtualDirectory out) {
	...
	public static void generateErrHandlerInterface(ErrorHandler eh,
			VirtualDirectory out) {
		String name = eh.getFunction();
		String target = eh.getTarget();
		String interfaceName = "I" + name;
	...
	public static void generateErrHandlerConfig(ErrorHandler eh,
			VirtualDirectory out) {
		String name = eh.getFunction();
		// String target = eh.getTarget();
		String interfaceName = "I" + name;
	...
	public static void generateErrHandlerModule(ErrorHandler eh,
			VirtualDirectory out) {
		String name = eh.getFunction();
		String target = eh.getTarget();
		String interfaceName = "I" + name;
	...
	public static void generateNodes(ProgramGraph g, VirtualDirectory out) {
		...
		while (it.hasNext()) {
			...
			if (isNodeAbstract(g, td.getName())) {
				...
				//if (td.getPlatformName().equals("MICA2DOT")) {
	...
	public static void generateRuntimeStdControlImpl(ProgramGraph g,
			VirtualDirectory out) 
	{
		...
		pw.println("TOSH_MAKE_INT0_OUTPUT();");
		pw.println("TOSH_CLR_INT0_PIN();");
		...
		for (ErrorHandler eh : errs) 
		{
			...
			{
				if (etd.getPlatformName().equals("MICA2DOT")) 
				{
		...
		/*while (it.hasNext()) 
		{
			...
			if (!isNodeAbstract(g, td.getName()) && td.getPlatformName().equals("MICA2DOT")) 
			{
				if (!g.isSource(td.getName())) 
				{
		...
		pw.println("result &= call SGStatusTimer.start(TIMER_REPEAT, 300);");
		pw.println("");
		pw.println("return result;");
		pw.println("}");
	...
	public static void generateRuntimeModuleHdr(ProgramGraph g,
			VirtualDirectory out) {
		Collection fns = g.getFunctions();
		Iterator it = fns.iterator();
		Program p = g.getProgram();
		...
		*/
		...
		out.getWriter(M_RUNTIMEM_NC).println("interface Timer as SGSleepTimer;");
		out.getWriter(M_RUNTIMEM_NC).println("interface Timer as SGStateTimer;");
		out.getWriter(M_RUNTIMEM_NC).println("interface Timer as SGCardTimer;");
		out.getWriter(M_RUNTIMEM_NC).println("interface Timer as SGStatusTimer;");
		...
		out.getWriter(M_RUNTIMEM_NC).println("interface SendMsg as StateSend;");
		out.getWriter(M_RUNTIMEM_NC).println("interface ReceiveMsg as PathRecv;");
		out.getWriter(M_RUNTIMEM_NC).println("interface ReceiveMsg as SleepRecv;");
	...
	public static void generateRuntimeModuleImpl(ProgramGraph g,
			VirtualDirectory out) {
		Collection fns = g.getFunctions();
		Iterator it = fns.iterator();
		Program p = g.getProgram();
	...
	public static void generateRuntimeModule(ProgramGraph g,
			VirtualDirectory out) {
		Collection fns = g.getFunctions();
		Iterator it = fns.iterator();
		Program p = g.getProgram();
	...
	public static void generateRuntimeConfig(ProgramGraph g,
			VirtualDirectory out) {
		Collection fns = g.getFunctions();
		Iterator it = fns.iterator();
		Program p = g.getProgram();
	...
	public static void generateRuntimeConfigHdr(ProgramGraph g,
			VirtualDirectory out) {
		Collection fns = g.getFunctions();
		Iterator it = fns.iterator();
		Program p = g.getProgram();
		...
		out.getWriter(M_RUNTIME_NC).println("includes rt_structs;\n");
		out.getWriter(M_RUNTIME_NC).println("configuration Runtime {");
		out.getWriter(M_RUNTIME_NC).println("}");

	}

	public static void generateRuntimeConfigImpl(ProgramGraph g,
			VirtualDirectory out) {
		Collection fns = g.getFunctions();
		Iterator it = fns.iterator();
		Program p = g.getProgram();
		...
		/*while (it.hasNext()) {
			...
			if (!isNodeAbstract(g, td.getName())
					&& td.getPlatformName().equals("MICA2DOT")) {
		...
		out.getWriter(M_RUNTIME_NC).println(
				"Main, TimerC, RuntimeM, BAllocC, Evaluator, PageEEPROMC, HPLPowerManagementM as Power,");
		...
		out.getWriter(M_RUNTIME_NC).println("components GenericComm as Comm;");
		out.getWriter(M_RUNTIME_NC).println("Main.StdControl -> Comm;");
		out.getWriter(M_RUNTIME_NC).println("RuntimeM.StateSend -> Comm.SendMsg[0xd1];");
		out.getWriter(M_RUNTIME_NC).println("RuntimeM.PathRecv -> Comm.ReceiveMsg[0xd2];");
		out.getWriter(M_RUNTIME_NC).println("RuntimeM.SleepRecv -> Comm.ReceiveMsg[0xd3];");
		...
		out.getWriter(M_RUNTIME_NC).println(
				"Main.StdControl -> BAllocC.StdControl;");
		out.getWriter(M_RUNTIME_NC).println(
		...
		out.getWriter(M_RUNTIME_NC).println(
				"RuntimeM.BAlloc -> BAllocC.BAlloc;");
		...
		out.getWriter(M_RUNTIME_NC).println(
			"RuntimeM.SGSleepTimer -> TimerC.Timer[unique(\"Timer\")];");
		out.getWriter(M_RUNTIME_NC).println(
			"RuntimeM.SGStateTimer -> TimerC.Timer[unique(\"Timer\")];");
		out.getWriter(M_RUNTIME_NC).println(
		"RuntimeM.SGStatusTimer -> TimerC.Timer[unique(\"Timer\")];");
		out.getWriter(M_RUNTIME_NC).println(
		"RuntimeM.SGCardTimer -> TimerC.Timer[unique(\"Timer\")];");
		out.getWriter(M_RUNTIME_NC).println(
	...
	public static void generateNodeDoneHandler(TaskDeclaration td,
		...
		out.println("__release_node_lock("+td.getName().toUpperCase() + "_NODEID);");
		out.println("now = call SysTime.getTime32();");
		// out.print("factorTime("+td.getName().toUpperCase()+"_NODEID,");
		// out.println("(*out)->_pdata.starttime, now);");
		out.println("if (error != ERR_OK)");
		out.println("{");
		out.println("call BAlloc.free((Handle)out);");
		out.print("return handle_error(" + td.getName().toUpperCase()
				+ "_NODEID,");
		...
		out.println("{");
		out.println("call BAlloc.free((Handle)out);");
		out.println("return handle_error(" + td.getName().toUpperCase()
				+ "_NODEID,NULL,ERR_FREEMEM);");
		out.println("}");
		...
		while (ei.hasNext()) {
			...
			if (destnode.getNodeType() != GraphNode.ERROR
				...
				if (dest == g.getVertexExit()) {
					...
					//TODO: Remove this for smaller code size
					if (plat == null || plat.equals("MICA2DOT")) {
						out.print("TRUE");
					} else {
						out.print("FALSE");
					}
	...
	public static void generateSrcFiredHandler(TaskDeclaration td,
			ProgramGraph g, PrintWriter out) {
	...
	public static void generateNodeDoneHandlers(ProgramGraph g,
			VirtualDirectory out) {
		Collection fns = g.getFunctions();
		Iterator it = fns.iterator();
		Program p = g.getProgram();
	...
	public static void generateErrDoneHandlers(ProgramGraph g,
			VirtualDirectory out) {
		Vector<ErrorHandler> errs = g.getProgram().getErrorHandlers();
	...
	public static String getMinPathState(ProgramGraph g, Vector<Vertex> path,
			int endidx) {
		int minState;
	...
	public static boolean isEdgeLocal(Vertex a, Vertex b, ProgramGraph g) {
		...
		if (!adj) {
			System.out.println(a.element().toString()
					+ " is not connected to: " + b.element().toString());
			System.exit(1);
		}
	...
	public static void generatePathEnergyCalculators(PrintWriter pw,
	...
	public static void generateNodesHeader(ProgramGraph g,
	...
	public static void generateAbstractDeciders(ProgramGraph g,
			VirtualDirectory out) {
		Collection fns = g.getFunctions();
		Iterator it = fns.iterator();
		Program p = g.getProgram();
	...
	private static int getEdgeWeight(ProgramGraph g, String source,
	...
	public static void generateCallHandlers(ProgramGraph g, VirtualDirectory out) {
		...
		out.getWriter(M_CALLS_H).println(
				"uint16_t translateID(EdgeIn *e, uint16_t *wt)");
		out.getWriter(M_CALLS_H).println("{");
		...
		out.getWriter(M_CALLS_H).println("}//switch");
		out.getWriter(M_CALLS_H).println(
				"return NO_NODEID; //should never happen");
		...
		//		.getWriter(M_CALLS_H)
		//		.println(
		//				"result = call BAlloc.allocate ((HandlePtr)&(e->outvar), outsize);");
		//out.getWriter(M_CALLS_H).println("if (result == FAIL)");
		//out.getWriter(M_CALLS_H).println("{");
		//out
		//		.getWriter(M_CALLS_H)
		//		.println(
		//				"return handle_error (e->node_id, (Handle)e->invar, ERR_NOMEMORY);");
		//out.getWriter(M_CALLS_H).println("}");

		// on your mark...
		//out.getWriter(M_CALLS_H).println("((GenericNode*)(*(e->invar)))->_pdata.starttime = call LocalTime.read();");
		out.getWriter(M_CALLS_H).println("((GenericNode*)(*(e->invar)))->_pdata.starttime = call SysTime.getTime32();");
		out.getWriter(M_CALLS_H).println(
						"((GenericNode*)(*(e->outvar)))->_pdata.starttime = ((GenericNode*)(*(e->invar)))->_pdata.starttime;");
		out.getWriter(M_CALLS_H).println(
						"((GenericNode*)(*(e->outvar)))->_pdata.sessionID = ((GenericNode*)(*(e->invar)))->_pdata.sessionID;");
		out
				.getWriter(M_CALLS_H)
				.println(
						"((GenericNode*)(*(e->outvar)))->_pdata.weight = ((GenericNode*)(*(e->invar)))->_pdata.weight;");
		...
		while (it.hasNext()) {
			...
			if (!isNodeAbstract(g, td.getName()) && !g.isSource(td.getName())
				...
				out.getWriter(M_CALLS_H).print(
						"(" + td.getName() + "_in**)e->invar,");
				out.getWriter(M_CALLS_H).println(
						"(" + td.getName() + "_out**)e->outvar);");
			}
		}
		out.getWriter(M_CALLS_H).println("}//switch");
	...
	public static void generateStruct(TaskDeclaration td, PrintWriter out,
	...
	public static void generate(String root, ProgramGraph pg, Program pm,
	...
	/*
	 * protected static int getStageNumber(String name) { Integer stg =
	 * stageNumber.get(name); if (stg == null) { stg = new Integer(totalStages);
	 * totalStages++; stageNumber.put(name, stg); } return stg.intValue(); }
	 */

	/*
	 * public static void generateSimpleStatement (SimpleFlowStatement sfs,
