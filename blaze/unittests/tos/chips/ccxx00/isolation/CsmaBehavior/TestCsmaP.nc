...
implementation {
  ...
  event void TestCcaRequest.run() {
    ...
    assertEquals("Send.send() failed", SUCCESS, call Send.send(&myMsg, 20));
  }
  ...
  /***************** Csma Events ****************/
  async event void InitialBackoff.requestBackoff[am_id_t amId](message_t *msg) {
    ...
    switch(call State.getState()) {
      ...
      case S_TESTINITIALREQUEST:
        call InitialBackoff.setBackoff[amId](0);
        break;
  ...
  async event void CongestionBackoff.requestBackoff[am_id_t amId](message_t *msg) {
    ...
    switch(call State.getState()) {
      ...
      case S_TESTSINGLECONGESTION:
        call CongestionBackoff.setBackoff[amId](0);
        break;
  ...
  async command error_t AsyncSend.send(void *msg, bool force, uint16_t preambleDurationMs) {
    asyncSendAttempts++;
