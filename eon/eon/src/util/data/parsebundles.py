...
def parsebundle(bundle):
	if (len(bundle) > 1):
		...
		toks = bundle[0].split()
		outstr = "";
		print ("toks",toks, str(bundle[0]))
		outstr = "<bundle turtle=\""+toks[1]+"\" bundle=\""+toks[3]+"\">\n";
		if (len(bundle) == 3):
			outstr = outstr + parsegpsbundle(bundle[1:]);
		else:
			outstr = outstr + parsertbundle(bundle[1:]);
