...
static inline int16_t dequantize(int8_t _val, uint8_t qval, uint8_t quality, uint8_t isDC)
{
...
void idctNew(uint8_t blkIdx_i, uint8_t blkIdx_j, uint16_t numBlks,
						int8_t* dct_data, uint8_t* out, uint8_t bytesPerPixel,
						uint8_t *qtable, uint8_t quality, uint16_t pixelsPerRow)
{
  ...
  uint8_t *outptr = &out[(pixelsPerRow*(blkIdx_j<<3)+(blkIdx_i<<3))*bytesPerPixel];
  ...
  for (ctr = DCTSIZE; ctr > 0; ctr--) {
    ...
    tmp0 = dequantize(inptr[numBlks*0], quantptr[DCTSIZE*0], quality,
                      (ctr==DCTSIZE)?1:0);
    tmp1 = dequantize(inptr[numBlks*2], quantptr[DCTSIZE*2], quality,0);
    tmp2 = dequantize(inptr[numBlks*4], quantptr[DCTSIZE*4], quality,0);
    tmp3 = dequantize(inptr[numBlks*6], quantptr[DCTSIZE*6], quality,0);
    ...
    tmp4 = dequantize(inptr[numBlks*1], quantptr[DCTSIZE*1], quality,0);
    tmp5 = dequantize(inptr[numBlks*3], quantptr[DCTSIZE*3], quality,0);
    tmp6 = dequantize(inptr[numBlks*5], quantptr[DCTSIZE*5], quality,0);
    tmp7 = dequantize(inptr[numBlks*7], quantptr[DCTSIZE*7], quality,0);
