...
module RadioMsgM
{
  ...
  uses
  {
    ...
    // Timers
    interface Timer<TMilli> as TimerWait; //slow down the photo sending
...
/* ---------------- RADIO REMOTE COMMANDS -------------------*/
  event message_t *ReceiveRadioCmd.receive(message_t *msg, void *payload, uint8_t len) {
   ...
   if(cmdMsg->cmd == 0x08 && VIDEO_ON==0){
    ...
    //stop video
    if(cmdMsg->cmd == 0x10 && VIDEO_ON==1){
	VIDEO_ON=0;
    }

    call cameraModule.radio_command_manager(cmdMsg);
 ...
 command error_t RadioMsg.sendPhotoRadio(uint8_t *start_buf, uint32_t size)
  ...
  task void processVideoFrameRadio()
  {
	...
	if(busy==0){
        ...
        //NOACK for the reconstructed frame
#ifdef ACK_DISABLED
  ...
  event void RadioVideoSend.sendDone(message_t* bufPtr, error_t error)
  {
        ...
        //NOACK for reconstructed frames
#ifdef ACK_DISABLED
       //if((error!=SUCCESS || (msgData->frame_id==0 && !call Ack.wasAcked(bufPtr))) && rtx_count<MAX_RTX) {     
#else
