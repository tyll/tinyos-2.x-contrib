...
#include "rt_structs.h"
  ...
  void doCalc ()
  {
	...
	current_c64 = current_c;
	netcurrent64 = netcurrent;
    g_temp = temperature;
	g_volts = voltage;
    ...
    //convert to uW
    discharge = (voltage_c * current_c64) / 1000;
	netcharge = (voltage * netcurrent64) / 1000;
	dismW = current_c64;
	netmW = netcurrent64;
  ...
  void BlinkLikeCrazy()
  {
	...
	command uint64_t IAccum.getInMicroJoules()
  	{
  		uint32_t now_time;
  		uint64_t result;
  		uint32_t elapsed;
  ...
  command uint64_t IAccum.getOutMicroJoules()
  {
  		uint32_t now_time;
  		uint64_t result;
  		uint32_t elapsed;
