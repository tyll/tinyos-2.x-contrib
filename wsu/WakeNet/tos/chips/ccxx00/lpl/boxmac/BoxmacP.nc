...
implementation {
  ...
  event void SendNotifier.aboutToSend[am_id_t amId](am_addr_t dest, message_t *msg) {
    ...
    if((sleepInterval = call LowPowerListening.getRemoteWakeupInterval(msg)) > 0) {
      call LowPowerListening.setRemoteWakeupInterval(msg, (sleepInterval / BOXMAC_WAKEUP_TRANSMISSION_DIVISIONS));
  ...
  event void AMSend.sendDone[am_id_t amId](message_t *msg, error_t error) {
    if(msg == myMsg) {
      call LowPowerListening.setRemoteWakeupInterval(msg, ((call LowPowerListening.getRemoteWakeupInterval(msg)) * BOXMAC_WAKEUP_TRANSMISSION_DIVISIONS));
