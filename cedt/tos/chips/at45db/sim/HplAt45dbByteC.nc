...
implementation 
{
  ...
  uint8_t mem_data[AT45_MAX_PAGES][AT45_PAGE_SIZE];
  ...
  uint8_t dontCare;
  at45page_t reqMemPage;
  at45pageoffset_t reqMemOffset;
	...
	for (;;)
	  {
		...
		if (!count)
	      ...
	      {
		*ptr++ = mem_data[reqMemPage][reqMemOffset];
        dbg("HplAt45dbByteRW","Reading %hhx\n",mem_data[reqMemPage][reqMemOffset]);
    	reqMemOffset++;
		--count;
		if (!count)
		  break;
		...
		if (!count)
		  break;
		mem_data[reqMemPage][reqMemOffset] = *ptr++;
        dbg("HplAt45dbByteRW","Written as %hhx count = %d\n",mem_data[reqMemPage][reqMemOffset],count);
        reqMemOffset++;
		--count;
	      }
	    else /* P_COMMAND */
  ...
  command void HplAt45db.read(uint8_t cmd,
	...
	dbg("HplAt45dbByteC","Called %d bytes to read in page %d \n",count,page);
	call At45dbSimEnergyEstimator.readEnergy(count);
    execCommand(P_READ, cmd, 5, page, offset, pdata, count);
  }

  command void HplAt45db.readBuffer(uint8_t cmd, at45pageoffset_t offset,
				    uint8_t *pdata, at45pageoffset_t count) {
	dbg("HplAt45dbByteC","Called %d bytes to read buffer in page %d\n",count,0);
	call At45dbSimEnergyEstimator.readEnergy(count);
  ...
  command void HplAt45db.write(uint8_t cmd,
			       at45page_t page, at45pageoffset_t offset,
			       uint8_t *pdata, at45pageoffset_t count) {
	dbg("HplAt45dbByteC","Called %d bytes to write in page %d with offset %d\n",count,page,offset);
	call At45dbSimEnergyEstimator.writeEnergy(count);
