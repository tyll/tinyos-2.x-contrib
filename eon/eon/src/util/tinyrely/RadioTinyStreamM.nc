  ...
  *********************************************/
  ...
  command result_t StreamWrite.write(uint8_t connid,
      ...
      if (bytesfree < size)
	{
      ...
      /*
      //replace with memcpy if too slow.  Shouldn't be a problem
      for (i=0; i < howmany; i++)
	{
	  buffers[connid].txdata[(buffers[connid].txtail + i) % TS_BUF_LENGTH] = buffer[i];
	}

      //adjust tail;
      buffers[connid].txtail = (buffers[connid].txtail + howmany) % TS_BUF_LENGTH;
      */
