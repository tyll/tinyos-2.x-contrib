...
def parsertbundle(bundle):
	...
	print "New bundle (len="+str(len(bundle))+")"
	print(bundle)
	#Singleton data
	...
	while not done:
		print "-->"+str((o_idx,b_idx,len(bundle)));
		if (o_idx < len(bundle[b_idx])):
			prob = bundle[b_idx][o_idx];
			o_idx = o_idx + 1;
	...
	if (linesleft % 2 != 0):
		...
		print(scanlines,b_idx, len(bundle));
		o_idx = 0;
		path=0;
		done = False;
		while not done:
			if (o_idx < len(bundle[b_idx])):
				...
				print(b_idx,scanlines,o_idx, len(bundle[b_idx]), len(bundle[b_idx+scanlines]))
				print bundle[b_idx+scanlines]
...
def parsegpsbundle(bundle):
	...
	outstr = outstr + "<longdec>"+str(((longmin * 1.0)/600000.0) + longdeg)+"</longdec>\n"
...
def parsebundle(bnum, bundle):
	if (len(bundle) > 1):
		print("Bundle",bnum)
		outstr = "<bundle bnum=\""+str(bnum)+"\" type=\"";
		if (len(bundle) != 12):
			outstr = outstr + "gps\">"
			outstr = outstr + parsegpsbundle(bundle);
		else:
			outstr = outstr + "rtdata\">"
			outstr = outstr + parsertbundle(bundle);
		outstr = outstr + "</bundle>\n\n";
		return outstr;
	else:
		return "***Empty Bundle***"
...
for line in rawdata:
	...
	if line.startswith("Turtle:"):
		print line
		toks = line.split()
		turtlenum = int(toks[1]);
		bundlenum = int(toks[3]);
		
	elif line.startswith("01"):
		print(line)
		chunknum,chrs = parsedataline(line);
		if bundlenum not in bundles:
			bundles[bundlenum] = {}
		if chunknum not in bundles[bundlenum]:
			bundles[bundlenum][chunknum] = chrs;
	
		
print "got the bundles..."
...
for b in bundles.keys():
	...
	thebundle = bundles[b];
	thenewbundle = []
	for chunk in thebundle:
		print "a chunk"
		print(thebundle[chunk])
		thenewbundle.append(thebundle[chunk]);
	thenewbundle.reverse();
	bundles[b] = thenewbundle;
	print "after"
	print bundles[b]

ofile.write("<turtledata>\n");
for b in bundles:
	#print("Page: " + str(p["page"])+"\n");

	ofile.write(parsebundle(b,bundles[b]));
