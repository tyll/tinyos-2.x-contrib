...
module BeaconBroadcasterP @safe() {
  uses {
    interface Boot;
...
implementation
{
  ...
  event void RadioControl.startDone(error_t error) {
    if (error == SUCCESS) {
      radioFull = FALSE;
      call Timer.startPeriodic(10480);
    }
  }
  ...
  event void AMSend.sendDone(message_t* msg, error_t error) {}
	...
	event void Timer.fired() {
		BeaconMsg_t *msg = (BeaconMsg_t*) call RadioPacket.getPayload(&buf, sizeof(BeaconMsg_t));
			
		msg->srcAddr = TOS_NODE_ID;
		msg->beaconId = beaconId++;
			
		call AMSend.send(AM_BROADCAST_ADDR, &buf, sizeof(BeaconMsg_t));
